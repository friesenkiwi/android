...
public class ServiceLocator
    implements ProxyableService, LostApiClient.ConnectionCallbacks, LocationListener {
  public static final String RECEIVER_ACTION_GEOFENCE_TRANSITION =
      "org.owntracks.android.RECEIVER_ACTION_GEOFENCE_TRANSITION";
  public static final String RECEIVER_ACTION_PUBLISH_LASTKNOWN_MANUAL =
      "org.owntracks.android.RECEIVER_ACTION_PUBLISH_LASTKNOWN_MANUAL";
  ...
  @Override public void onCreate(ServiceProxy p) {
    this.context = p;
    ...
    Preferences.registerOnPreferenceChangedListener(new Preferences.OnPreferenceChangedListener() {
      @Override public void onAttachAfterModeChanged() {

      }

      @Override public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (key.equals(Preferences.Keys.PUB) ||
  ...
  */
  ...
  public Location getLastKnownLocation() {
    ...
    if (hasConnectedApiClient()) {
      try {
        this.lastKnownLocation = LocationServices.FusedLocationApi.getLastLocation(apiClient);
  ...
  public void enteredWifiNetwork(String ssid) {
    ...
    List<Waypoint> ws = this.waypointDao.queryBuilder()
        .where(WaypointDao.Properties.ModeId.eq(Preferences.getModeId()), WaypointDao.Properties.WifiSSID.like("TestSSID"))
        .build()
        .list();
    ...
    if (event != null) {
      ...
      if (transition == Geofence.GEOFENCE_TRANSITION_ENTER
        ...
        for (int index = 0; index < event.getTriggeringGeofences().size(); index++) {
          ...
          Waypoint w = this.waypointDao.queryBuilder()
              .where(WaypointDao.Properties.GeofenceId.eq(event.getTriggeringGeofences().get(index).getRequestId()))
              .limit(1)
              .unique();
  ...
  private boolean shouldPublishLocation() {
    if (!Preferences.getPub()) return false;
...
/*
  ...
  private void requestLocationUpdatesAsync() {
    ...
    if (!isReady() || !hasConnectedApiClient()) {
      Timber.e(
          "requestLocationUpdates but not ApiClient not connected. Updates will be requested again once connected");
      return;
    }
    ...
    try {
      ...
      if (this.foreground) setupForegroundLocationRequest();
      else setupBackgroundLocationRequest();
  ...
  @Override public void onStartCommand(Intent intent, int flags, int startId) {
  ...
  @Subscribe public void onEvent(Events.Dummy event) {
  ...
  @Override public void onLocationChanged(Location location) {
    ...
    if (shouldPublishLocation()) reportLocation();
  }
  ...
  private void reportLocation(String trigger) {
    ...
    message.setTst(TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()));
    message.setTid(Preferences.getTrackerId(true));
    if (Preferences.getPubLocationExtendedData()) message.setBatt(App.getBatteryLevel());

    ServiceProxy.getServiceMessage().sendMessage(message);
  }

  @SuppressWarnings("unused") @Subscribe(threadMode = ThreadMode.BACKGROUND) public void onEvent(Events.WaypointAdded e) {
    handleWaypoint(e.getWaypoint(), false, false);
  }

  @SuppressWarnings("unused") @Subscribe(threadMode = ThreadMode.BACKGROUND) public void onEvent(Events.WaypointUpdated e) {
    handleWaypoint(e.getWaypoint(), true, false);
  }

  @SuppressWarnings("unused") @Subscribe(threadMode = ThreadMode.BACKGROUND) public void onEvent(Events.WaypointRemoved e) {
    handleWaypoint(e.getWaypoint(), false, true);
  }

  @SuppressWarnings("unused") @Subscribe public void onEvent(Events.ModeChanged e) {
    removeGeofencesByWaypoint(loadWaypointsForModeId(e.getOldModeId()));
    requestGeofences();
  }

  @SuppressWarnings("unused") @Subscribe public void onEvent(Events.PermissionGranted e) {
  ...
  private void handleWaypoint(Waypoint w, boolean update, boolean remove) {
    ...
    if (update && remove) throw new IllegalArgumentException("update and remove cannot be true at the same time");
    ...
    if (update || remove) removeGeofence(w);
  ...
  private void requestGeofences() {
    ...
    for (Waypoint w : loadWaypointsForCurrentModeWithValidGeofence()) {
      ...
      Geofence geofence = new Geofence.Builder().setRequestId(w.getGeofenceId())
          //.setTransitionTypes(Geofence.GEOFENCE_TRANSITION_ENTER | Geofence.GEOFENCE_TRANSITION_EXIT)
          //                    .setNotificationResponsiveness(30*1000)
          .setCircularRegion(w.getGeofenceLatitude(), w.getGeofenceLongitude(), w.getGeofenceRadius()).setExpirationDuration(Geofence.NEVER_EXPIRE).build();
    ...
    try {
      LocationServices.GeofencingApi.addGeofences(apiClient, getGeofencingRequest(fences),
          ServiceProxy.getBroadcastIntentForService(context, ServiceProxy.SERVICE_LOCATOR,
              ServiceLocator.RECEIVER_ACTION_GEOFENCE_TRANSITION, null));
            /*
  ...
  private void handleSecurityException(@Nullable SecurityException e) {
    if (e != null) Timber.e(e.getMessage());
  ...
  private void removeGeofencesByWaypoint(List<Waypoint> list) {
    ...
    for (Waypoint w : list == null ? loadWaypointsForCurrentMode() : list) {
      if (w.getGeofenceId() == null) continue;
  ...
  private void removeGeofencesById(List<String> ids) {
    ...
    LocationServices.GeofencingApi.removeGeofences(apiClient, ids);
/*
